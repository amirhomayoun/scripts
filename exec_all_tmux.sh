...
function execute_in_all_panes {
  ...
  for (( window=1; window <= $windows; window++ )); do
    ...
    for (( pane=1; pane <= $panes; pane++ )); do
      ...
      #echo $command
      tmux send-keys C-c C-c " $command && fg 2>/dev/null" C-m
    done
  done
...
#execute_in_all_panes "echo OHH"
#execute_in_all_panes "ls"
execute_in_all_panes "tmup"
